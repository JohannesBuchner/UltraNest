...
def update_clusters(points, clusterids, maxradiussq):
    ...
    returns the number of clusters found, which is also clusterids.max()
    overwrites points and clusterids in-place
    """
    currentclusterid = 1
    ...
    return nclusters, overlapped_points
...
class ScalingLayer(object):
    ...
    def update(self, upoints, maxradiussq):
        ...
        nclusters = update_clusters(points, clusteridxs, maxradiussq)
        self.nclusters = nclusters
        self.fast_update(points)
...
class AffineLayer(ScalingLayer):
    ...
    def fast_update(self, points):
        ...
        print("transform:", self.T)
        self.clusterids = np.ones(len(points), dtype=int)
    
    def transform(self, u):
        #uu = np.array([np.dot(self.T, p - self.ctr) for p in u])
        #return uu
