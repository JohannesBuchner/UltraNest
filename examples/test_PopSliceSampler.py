...
def main(args):
    ...
    if args.SimSlice:
        ...
        sampler.stepsampler = ultrapop.PopulationSimpleSliceSampler(popsize=args.popsize,nsteps=args.nstep,generate_direction=direction[args.direction],scale=1.0,scale_adapt_factor=1.0)
    if args.PopSlice:
        import ultranest.popstepsampler as ultrapop
        direction=[ultrapop.generate_cube_oriented_direction,ultrapop.generate_mixture_random_direction,ultrapop.generate_differential_direction,ultrapop.generate_region_random_direction,ultrapop.generate_region_oriented_direction,ultrapop.generate_random_direction]
    ...
    sampler.run(frac_remain=0.5, min_num_live_points=args.num_live_points, max_num_improvement_loops=3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rosenbrock', action='store_true')
    parser.add_argument('--multishell', action='store_true')
    parser.add_argument('--gaussian', action='store_true')
    parser.add_argument('--sigma', type=float, default=1)
    parser.add_argument('--eggbox', action='store_true')
    parser.add_argument('--funnel', action='store_true')
    parser.add_argument('--SimSlice', action='store_true')
    parser.add_argument('--PopSlice', action='store_true')
    parser.add_argument('--Slice', action='store_true')
    parser.add_argument('--PopGaussWalk', action='store_true')
